...
async function loadConfig() {
  const d = await (await api("/admin/config")).json();
  CONFIG = d;
  $("rate").textContent = (d.krwPerUsd || 1500).toLocaleString();
  const sel = $("modelSel");
  sel.innerHTML = "";
  (d.models || []).forEach((m) => {
    const o = document.createElement("option");
    o.value = m; o.textContent = m;
    if (m === d.geminiModel) o.selected = true;
    sel.appendChild(o);
  });
  $("curModel").textContent = d.geminiModel;
}
async function saveModel() {
  ...
  try {
    const d = await (await api("/admin/config", { method: "POST", body: JSON.stringify({ geminiModel: model }) })).json();
    $("curModel").textContent = d.geminiModel || model;
    msg.textContent = "✓ 저장됨";
...
async function loadCosts() {
  RAWCOSTS = await (await api("/admin/costs")).json();
...
async function loadFiles() {
  const d = await (await api("/admin/files")).json();
  FILES = d.items || [];
  renderFiles();
}
...
async function loadAll() {
  showDash();
  try { await Promise.all([loadConfig(), loadCosts(), loadFiles()]); }
...
const WORKFLOW_INTRO = `
  <div>
    ...
    <p class="leading-relaxed">단일 LLM 대화 + <b>함수 호출(function calling)</b>로 과정안을 완성합니다. 정확성이 필요한 교육과정 데이터는 RAG 함수로 조회하고, 화면 표시·확정은 UI/제어 함수로 처리합니다. 메인(<code>/</code>)은 Interactions API(서버 stateful), <code>/chat</code>은 기존 generateContent 방식입니다(둘 다 같은 시스템 프롬프트·함수 사용).</p>
  </div>
  <div>
  ...
  <div>
    ...
    <ul class="list-disc pl-5 space-y-0.5 text-slate-600 leading-relaxed">
      ...
      <li><b>두 버전 병행</b> — 메인(<code>/</code>)=Interactions API(서버 stateful), <code>/chat</code>=기존 generateContent</li>
...
async function loadWorkflowDoc() {
  ...
  try {
    ...
    const src = await (await fetch("./app.js?t=" + Date.now(), { cache: "no-store" })).text();
    const m = src.match(/const SYSTEM_PROMPT = `([\s\S]*?)`;/);
    const prompt = m ? m[1] : "(시스템 프롬프트를 추출하지 못했습니다)";
    body.innerHTML = WORKFLOW_INTRO +
      `<div>
        <h3 class="font-semibold text-slate-800 mb-1">실제 시스템 프롬프트 (전문 · app.js에서 추출)</h3>
