...
/* ── 모델: /35는 3-Tier 라우팅 (PRIMARY=gemini-3.5-flash 평가·전개활동, MID/LITE=gemini-2.5-flash, 예외 stage 11=gemini-3-flash-preview) ──
...
const WORKFLOW_INTRO = `
  ...
  <div>
    ...
    <ul class="list-disc pl-5 space-y-0.5 text-slate-600 leading-relaxed">
      ...
      <li><b>3-Tier 라우팅 (/35)</b> — PRIMARY <code>gemini-3.5-flash</code>(단계 6·9·10), MID <code>gemini-2.5-flash</code>(단계 5·7·검수·최종검토), 예외 <code>gemini-3-flash-preview</code>(단계 11 수업자의도), LITE <code>gemini-2.5-flash</code> 8K(단계 1~4·8). 알 수 없는 stage는 MID로 안전 폴백. 단계 충돌·MALFORMED·JSON 파싱 실패 시 한 tier 자동 격상(최대 1회). <code>FORCE_PRIMARY=true</code> 또는 <code>STAGE6_/STAGE11_/VERIFY_FORCE_PRIMARY</code>로 즉시 롤백</li>
      ...
      <li><b>완료 검토</b> — 검수 1차(99 A) <code>gemini-2.5-flash-lite</code>, 2차(99 B)·최종검토(100) <code>gemini-2.5-flash</code>로 호출, JSON 파싱 실패 시 한 tier 자동 폴백. 회귀 시 <code>VERIFY_FORCE_PRIMARY=true</code>로 즉시 PRIMARY 복원</li>
