...
- 시간(분)은 숫자로 적습니다(통합 키 "도입_시간"/"전개_시간"/"정리_시간"). 8단계 이후에 교수·학습 활동 상세 작성 규칙(위계 기호·sub 키·자료유의평가 형식)을 별도로 안내합니다.
...
const STAGE_GUIDES = {
  ...
  6: `평가 계획 (백워드 설계 — ⓪~③을 하나씩 present_choices로 확정하며 진행). 상세는 진입(⓪~②) / 범주 반복(③) 별로 별도 블록으로 안내합니다.`,
...
// 옵션 3: stage 6(평가) 가이드 분할 — 진입(⓪~②)과 범주 반복(③) 블록을 별도로 두고, 현 진행 시점에 맞는 블록만 system 에 주입한다.
const STAGE6_INTRO = `[6단계 진입 — 백워드 ⓪~②]
   ★ 진입 안내: 탐구 질문 직후 ⓪ 카드를 띄우기 전에, 백워드 설계를 시작한다는 안내를 한 번 보냅니다. '영속적 이해'·'수용 가능한 증거'는 생소하니 풀어서 안내하세요. 예: "평가 계획을 백워드 설계로 먼저 세워 볼게요. 학생이 무엇을 이해하고 할 수 있어야 하는지 정한 뒤 그 확인 방법을 정하는 방식이에요."
   ...
   ※ ⓪~①은 문서 칸이 없으니 present_choices로 확정만 받고 update_plan하지 않습니다(② 수행 과제만 반영). 수행 과제 확정 직후 ③ 평가 범주 단계로 이어집니다.`;

const STAGE6_CATEGORY = `[6단계 반복 — 평가 범주 ③]
   ③ 평가 범주: present_choices(field=평가범주, options=["지식·이해","과정·기능","가치·태도"], multi=true)로 받고, 고른 각 범주(i=1,2,…)마다 아래 ㄱ→ㄴ→ㄷ→ㄹ을 **한 번에 하나씩** 진행합니다 — 한 항목을 present_choices(또는 update_plan)로 처리하고 그 결과(사용자 선택·반영)를 받은 뒤에만 다음 항목으로 넘어갑니다. 한 응답에 한 항목만 다루고(특히 성취수준 '상'을 present_choices로 받은 뒤에 피드백으로 넘어갑니다), 성취수준·피드백을 한꺼번에 몰아 update_plan하지 않습니다:
      ㄱ. 평가요소: present_choices(field="평가요소", allow_custom=true) → update_plan("평가{i}_요소"). 평가요소는 명사형(동명사 '~기')으로 적습니다(예: "한글 자모의 이름과 소릿값 알기", "수평잡기 활동으로 물체 무게 비교하기"). 한 범주에 여럿이면 명사형으로 각각 쉼표·줄바꿈 구분. 사용자가 직접 입력해도 명사형으로 다듬어 반영합니다.
      ...
      ㄷ. 성취수준: '상' 기준만 present_choices(allow_custom)로 받고, '중'·'하'는 자동 작성 → 한 번의 update_plan으로 "평가{i}_수준상","평가{i}_수준중","평가{i}_수준하" 함께 반영.
         ★ 성취수준 진술(2022 개정): 상·중·하 모두 학생이 '도달한 정도와 할 수 있는 것'을 긍정적·기능적으로 진술하고, 세 수준을 능력의 범위·깊이·자립도로 구분합니다.
      ...
      ㄹ. 피드백 ('하' 수준 지원만): '하' 수준 학생이 다음 단계로 나아가도록 교사가 도울 방안을 update_plan("평가{i}_피드백")으로 반영합니다(상·중 칭찬·확장은 넣지 않음). 그 평가요소에서 어려운 부분을 기초부터 다시 안내·예시·단계적 발문으로 돕는 구체적 지원을 '~하도록 한다/안내한다' 형태의 한 문장으로, 수준 표시 없이 지원 내용만 적습니다('(하)' 접두 없음). 예: '합동과 관련된 수학 용어를 다시 상기시키고 설명해 보도록 한다.'
      평가 키는 범주가 1개여도 인덱스를 붙이고("평가1_범주","평가1_요소","평가1_방법","평가1_수준상","평가1_수준중","평가1_수준하","평가1_피드백"), update_plan에 "평가_num"(범주 수)을 함께 넣습니다.`;

// stage 6 진행 시점에 따라 어떤 블록을 주입할지 결정. self=stage 6 자기 자신인지(곁 윈도우면 false).
function pickStage6Body(p, self) {
  p = p || {};
  const hasTask = !!(p["수행과제"] && String(p["수행과제"]).trim());
  const hasCat = !!(p["평가_num"] || p["평가1_범주"]);
  if (!self) {
    // 곁 윈도우(s===5 또는 s===7).
    if (hasCat) return ""; // stage 7 곁: 평가 이미 진행/완료 → stage 6 가이드 전체 생략(추가 절감)
    return STAGE6_INTRO;   // stage 5 곁: 진입 안내만
  }
  if (!hasTask) return STAGE6_INTRO;             // ⓪~② 진행 중
  if (hasTask && hasCat) return STAGE6_CATEGORY; // ③ 진행 중
  return `${STAGE6_INTRO}\n\n${STAGE6_CATEGORY}`; // 전이 1턴: ② 직후 ③ 카드 직전(둘 다 주입해 안전)
}

function buildSystemPrompt(stage, partialPlan) {
  const s = Math.max(1, Math.min(11, parseInt(stage) || 1));
  const p = partialPlan || (typeof state !== "undefined" ? state.partialPlan : null) || {};
  ...
  const parts = window.map(n => {
    if (n === 6) {
      const body = pickStage6Body(p, n === s);
      return body ? `[6단계 세부]\n${body}` : "";
    }
    return `[${n}단계 세부]\n${STAGE_GUIDES[n]}`;
  }).filter(Boolean);
  const guides = parts.join("\n\n");
      ...
      if (!functionCalls.length) {
        // 순수 텍스트 응답 — 턴 종료(또는 재요청). 재요청을 유발한 '잘못된' 텍스트는 표시하지 않고 스피너만 유지.
        {
          ...
          if (choiceRetried < 2 && (_bulletCount >= 2 || _hasChoicePhrase)) {
            choiceRetried++;
            state.interInput = choiceRetried === 1
              ? "방금 안내한 항목의 선택지를 지금 present_choices 카드로 띄워 주세요(채팅에 번호·불릿으로 나열하지 말고). 도구는 본문 텍스트로 적지 말고 반드시 실제 함수 호출로 보내세요."
              : "다시 안내드립니다: 이번 턴의 후보를 반드시 present_choices 함수 호출로 카드를 띄워 주세요. 본문에 후보를 나열만 하는 응답은 금지입니다.";
            if (!loader) loader = addLoader();
            continue;
